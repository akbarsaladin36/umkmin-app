...
class SaleService implements SaleServiceInterface
{
    ...
    public function GetSalesService()
    {
        try {
            ...
            if(count($sales) > 0) {
                return Helper::GetResponse(200, 'All sales are succesfully appeared!', $sales);
            } else {
    ...
    public function GetSaleService($saleCode)
    {
        try {
            ...
            if($sale) {
                return Helper::GetResponse(200, 'A sale data is succesfully appeared!', $sale);
            } else {
    ...
    public function CreateSaleService($request)
    {
        ...
        try {
            ...
            return Helper::GetResponse(200, 'A new sales data is succesfully created!', $response);
        } catch(Throwable $e) {
            DB::rollBack();
            return Helper::GetResponse(500, $e->getMessage());
        }
        ...
        try {
            ...
            if($sale) {
                if($request->status == "paid") {
                    $authUser = Helper::GetAuthUser($request);
                    if(intval($request->paid_amount) < intval($sale->final_amount)) {
                        return Helper::GetResponse(400, 'Your money is not enough to pay this products!');
                    }
                    $changeAmount = intval($request->paid_amount) - intval($sale->final_amount);
                    $data = [
                    ...
                    foreach($saleItems as $item) {
                        $this->saleRepository->UpdateProduct($item->product_code, $item->quantity);
                    }
                } else {
                    $authUser = Helper::GetAuthUser($request);
                    $data = [
                        'status' => $request->status ? $request->status : $sale->status,
                        'updated_at' => date("Y-m-d H:i:s"),
                        'updated_by' => $authUser->uuid,
                        'updated_by_username' => $authUser->username
                    ];
                ...
                DB::commit();
                return Helper::GetResponse(200, 'A sale data status is succesfully updated!', $data);
            } else {
                DB::rollBack();
                return Helper::GetResponse(400, 'A sale data is not found!');
            }
    ...
    public function DeleteSaleService($saleCode)
    {
        try {
            ...
            if($sale) {
                if($sale->status == "paid") {
                    return Helper::GetResponse(400, 'A sale data cannot be deleted because status is paid!');
                } else {
                    $this->saleRepository->DeleteSaleItem($saleCode);
                    $this->saleRepository->Delete($saleCode);
                    return Helper::GetResponse(200, 'A sale data is succesfully deleted!');
                }
            } else {
                return Helper::GetResponse(400, 'A sale data is not found!');
            }
