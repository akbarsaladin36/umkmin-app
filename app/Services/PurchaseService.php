...
class PurchaseService implements PurchaseServiceInterface
{
    ...
    public function GetPurchasesService()
    {
        try {
            ...
            if(count($purchases) > 0) {
                return Helper::GetResponse(200, 'All purchases data are succesfully appeared!', $purchases);
            } else {
                return Helper::GetResponse(400, 'All purchases data are empty!');
            }
        } catch(Throwable $e) {
    ...
    public function GetPurchaseService($purchaseCode)
    {
        try {
            ...
            if($purchase) {
                return Helper::GetResponse(200, 'A purchase data is succesfully appeared!', $purchase);
            } else {
                return Helper::GetResponse(400, 'A purchase data is not found!');
            }
        } catch(Throwable $e) {
            return Helper::GetResponse(500, $e->getMessage());
        }
    ...
    public function CreatePurchaseService($request)
    {
        ...
        try {
            ...
            if($purchase) {
                DB::rollBack();
                return Helper::GetResponse(200, 'A purchase data is previously registered!');
            } else {
                foreach($request->purchase_items as $item) {
                    $purchaseItemCode = Helper::GenerateCode('purchase-items');
                    $subtotal = intval($item['quantity']) * intval($item['cost_price']);
                    $purchase_items[] = [
                ...
                return Helper::GetResponse(400, 'A new purchase data is succesfully created!', $response);
            }
        } catch(Throwable $e) {
            DB::rollBack();
            return Helper::GetResponse(500, $e->getMessage());
        ...
        try {
            ...
            if($purchase) {
                if($request->status == "paid") {
                    $authUser = Helper::GetAuthUser($request);
                    if(intval($request->paid_amount) < intval($purchase->total_amount)) {
                        return Helper::GetResponse(400, 'Your money is not enough to pay this products!');
                    }
                    $data = [
                    ...
                    $purchaseItems = $this->purchaseRepository->GetPurchaseItemsByPurchaseCode($purchaseCode);
                    foreach($purchaseItems as $item) {
                        $this->purchaseRepository->UpdateProduct($item->product_code, $item->quantity);
                    }
                } else {
                    $authUser = Helper::GetAuthUser($request);
                    $data = [
                ...
                DB::commit();
                return Helper::GetResponse(200, 'An existing purchase data status is succesfully updated!', $data);
            } else {
                DB::rollBack();
                return Helper::GetResponse(400, 'A purchase data is not found!');
            }
        ...
        try {
            ...
            if($purchase) {
                if($purchase->status == "paid") {
                    return Helper::GetResponse(400, 'A purchase data cannot be deleted because status purchase is paid!');
                } else {
                    $this->purchaseRepository->DeletePurchaseItem($purchaseCode);
                    $this->purchaseRepository->Delete($purchaseCode);
                    return Helper::GetResponse(200, 'A purchase data is succesfully deleted!');
                }
            } else {
                return Helper::GetResponse(400, 'A purchase data is not found!');
            }
