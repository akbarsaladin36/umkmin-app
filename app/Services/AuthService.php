...
class AuthService implements AuthServiceInterface
{
    ...
    public function RegisterService($request)
    {
        try {
            ...
            if($user) {
                return Helper::GetResponse(400, 'A user data is registered!', $user);
            } else {
                ...
                return Helper::GetResponse(200, 'A new user is succesfully registered!', $data);
            }
        } catch(Throwable $e) {
            ...
            if($user) {
                ...
                if($checkPassword) {
                    ...
                    $cookie = Cookie::make(
                        ...
                        '/',
                        null,
                        false,      // production: true kalau HTTPS
                        true,      // HttpOnly
                        false,
                        'lax'
                    );
                    return Helper::GetResponse(200, 'An user is succesfully login', $sessions)->withCookie($cookie);
                } else {
                    return Helper::GetResponse(400, 'A password did not match!');
                }
            } else {
    ...
    public function LogoutService($request)
    {
        try {
            ...
            return Helper::GetResponse(200, 'A user is succesfully logout!')->cookie('token', '', -1);
        } catch(Throwable $e) {
            return Helper::GetResponse(500, $e->getMessage());
        }
    }
